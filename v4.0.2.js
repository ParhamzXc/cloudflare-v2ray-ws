...
// ==================== 1. 全局配置 ====================
const 全局配置 = {
    密钥: "", // 【重要】请修改此处，这是你的连接密码，也是管理面板的路径
    默认兜底反代: "",
...
<body>
    <div class="container">
        ...
        <div class="input-group">
            <label>UUID (VLESS ID)</label>
            <input type="text" id="uuid" placeholder="Generated...">
        </div>

        <div style="border-top: 1px solid var(--border); margin: 24px 0;"></div>

        <div class="input-group">
            <label>SOCKS5 (Optional) - e.g. user:pass@1.1.1.1:443</label>
            <input type="text" id="s5" placeholder="Leave blank to disable">
        </div>

        <div class="input-group">
            <label>IP (Optional) - e.g. 1.1.1.1:443</label>
            <input type="text" id="pyip" placeholder="Leave bland to use default policy">
        </div>

        <button class="btn" onclick="generate()">Generate subscription configuration</button>
    ...
    <script>
        // Initialization: Key is automatically obtained from URL path, UUID is automatically generated.
        ...
        function generate() {
            // Retrieve the preferred URL provided by the user (default is www.shopify.com)
            const address = document.getElementById('address').value.trim();
            // Automatically obtain the current worker's real domain name as the SNI and Host.
            ...
            if (!address || !key || !uuid) { alert('Please complete the required fields.'); return; }
            ...
            // address -> Server address (www.shopify.com)
            // workerHost -> SNI and host (Your worker domain)
  ...
  ws-opts:
    ...
    headers:
      Host: \${workerHost}\`.trim();
            ...
            renderOutput('VLESS Link', vlessLink);
